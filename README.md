...
2. Install the AWS Amplify CLI and react-scripts:
...
4. Configure your AWS Applify to use the AWS account:
...
6. Create an AWS Lex v2 bot. Optionally, use the provided shell script `create_lex_bot.sh`:
...
7. Run the `update_chatbot_config.sh` script to update the `src/ChatBot.js` file with your Identity Pool ID, AWS Region, Bot ID, and Bot Alias ID:

   ```bash
   chmod +x update_chatbot_config.sh
   ./update_chatbot_config.sh
   ```

   This script will:

   - Read the AWS Region and Identity Pool ID from `src/amplifyconfiguration.json`
   - Prompt you to enter your Bot ID and Bot Alias ID
   - Update the `src/ChatBot.js` file with all these values

8. Add Lex access to the unauthenticated (guest) role:
   Make the script executable and run it:
